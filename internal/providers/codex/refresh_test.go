...
func TestCodexRefreshExchange(t *testing.T) {
	tests := []struct {
		name string
		...
		{"404 endpoint broken", func(t *testing.T) {
		...
		{"non oauth body", func(t *testing.T) {
			body := []byte(`{"status":"ok"}`)
			srv := stubRefreshServer(t, 200, body, nil)
			rc := newTestRefreshClient(t, srv)
		...
		{"503 transient", func(t *testing.T) {
		...
		{"network error", func(t *testing.T) {
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
		...
		{"expires at zero when expires_in absent", func(t *testing.T) {
		...
		{"post body form encoded", func(t *testing.T) {
		...
		{"401 and 403 are bare errors not auth denied", func(t *testing.T) {
			// codexRefreshClassify does not delegate to DefaultClassify, so these
			// statuses must NOT wrap ErrAuthDenied.
		...
		{"invalid client is endpoint broken not rejected", func(t *testing.T) {
