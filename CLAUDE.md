...
These are upstream OAuth-provider behaviors aistat cannot work around without re-authentication. Both fail closed with actionable errors. **The device-auth flow (`codex login --device-auth`) likely avoids both** because device-auth tokens are scoped to a unique device session on OpenAI's side, so logging another account in elsewhere does not revoke this device's tokens. Recommended for headless or multi-account use.

- **Refresh-token rotation race.** OpenAI's `/oauth/token` endpoint single-uses each refresh_token; the Codex CLI rotates it on every refresh and writes the new value to `~/.codex/auth.json`. If the Codex CLI runs between aistat reading the file and aistat sending its refresh request, aistat's in-memory copy is stale and the server returns `HTTP 401: refresh_token already used`. Recovery: `codex login` (or just wait for the next aistat run — the cache will catch up next pass).
- **Switch-side token revocation (browser-login flow).** When a new account logs in on the same OAuth client via the browser flow (`codex logout && codex login`), OpenAI's server invalidates the previous account's tokens (`token_revoked`). aistat's `switch` re-publishes the stored blob byte-for-byte, but server-revoked tokens stay revoked — the next usage call returns `HTTP 401: token_revoked`. Recovery: `codex login` for the now-active account. **On a remote or headless machine, prefer `codex login --device-auth` for each account; device-scoped tokens should sidestep the revocation.**
